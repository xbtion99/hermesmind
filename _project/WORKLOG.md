...
## 2026-09-18  Claude (claude-code-web / xbtion99) — 저장소 스위트 기준선 대조
- 목표: AGENTS.md가 요구하는 "푸시 전 전체 스위트 실행"을 그동안 건너뛴 것을 메운다
- 방법: venv가 없어 scripts/run_tests.sh를 쓸 수 없으므로 그 위생 환경(TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0, -n 4)을 재현. git worktree로 origin/main을 펼쳐 동일 명령으로 비교
- 검증 결과: main 2977실패/11060통과/935오류 · 브랜치 2회차 2977/11060/935로 일치 · 1회차의 1건 차이는 2회차에 사라짐(흔들리는 테스트) · 영향 범위 519개 비교는 양쪽 완전 동일 · 스킬 스캔 테스트 157개 통과 [샌드박스 검증됨]
- 결론: 이 브랜치는 저장소 테스트를 깨뜨리지 않는다. 2977개 실패는 샌드박스에 선택적 의존성이 없어서이며 main도 같다
- blocker: 없음
- 다음 한 가지 행동: 의존성이 갖춰진 환경에서 같은 비교를 재확인 (CI 또는 Mac 필요)
