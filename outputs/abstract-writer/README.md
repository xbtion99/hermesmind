...
위 예시에 셸 표시(`$`)를 일부러 넣지 않았다. 그대로 복사해 붙여넣어도 주제가 망가지지 않는다.
(혹시 `$`가 섞여 들어와도 프로그램이 떼어낸다.)
...
- **길이와 단락 수**: `--length`나 `--form`을 함께 줄 때만 검사한다. 한국어는 글자수, 영어는 단어수.
  ±10%는 봐준다. 함축 레지스터는 목표가 지정 길이의 40~60%로 줄어든다

```bash
python3 -m abstract_writer --lint draft.md --length medium --form 단상
```

`--lint`만 주면 길이는 보지 않는다. 브리프를 같이 줘야 브리프대로 썼는지 검사한다.
